...
3. Clone the ChatterGame repository:

    ```
    git clone https://github.com/salesforcelabs/chatter-game
    cd chatter-game
    ```

4. Create a scratch org and provide it with an alias (**ChatterGame** in the command below):

    ```
    sfdx force:org:create -s -f config/project-scratch-def.json -a ChatterGame
    ```
...
6. Assign the `ChatterGame` permission set to the default user.

    ```
    sfdx force:user:permset:assign -n ChatterGame
    ```
...
8. In App Launcher, click **View All** then select the **ChatterGame** app.
...
7. In App Launcher, select the **ChatterGame** app.
